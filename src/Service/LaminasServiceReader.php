...
class LaminasServiceReader implements ServiceReaderInterface
{
    ...
    private function getServiceInfo(string $serviceName): ServiceInfo
    {
        ...
        if ($this->container->hasFactory($serviceName)) {
            ...
            $factory = is_object($factoryObj) ? get_class($factoryObj) : (string) $factoryObj;
        }

        // Check if it's an invokable
        if ($this->container->hasInvokableClass($serviceName)) {
            $type = 'invokable';
            $invokableClass = $this->container->getInvokableClass($serviceName);
        }
